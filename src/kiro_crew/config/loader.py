...
def session_project_dir(session_key: str, root: str = "", workspace: str | None = None) -> str:
    ...
    * Consequently the directory is created EXCLUSIVELY, and an existing one is
    ...
    try:
        ...
        try:
            if can_pin:
                root_pin = pinned_fs.pin_parent(str(base_resolved), what="per-session project root")
                try:
                    os.mkdir(name, 0o700, dir_fd=root_pin)
                finally:
                    os.close(root_pin)
            else:
                root_pin = platform_compat.pin_directory(base_resolved)
                try:
                    # ORDER IS THE PROPERTY here, not the two calls. Re-reading
                    # the ancestor shape while the pin is HELD is what makes it
                    # decisive: a swap that landed before the pin gives a
                    # different answer than validation saw, and a swap after it
                    # cannot happen, because the handle forbids renaming or
                    # deleting the root and everything above it. The same
                    # comparison made before the pin leaves that window open, and
                    # refusing every linked ancestor outright rejects a root
                    # under a redirected profile or a mapped drive.
                    if platform_compat.first_linked_ancestor(base_resolved) != linked_ancestor:
                        return ""
