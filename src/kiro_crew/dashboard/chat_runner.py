...
def _append_redaction_notice(slot: _ChatSlot, redacted: str) -> None:
    ...
    Tell the user the text was altered. The notice is mandatory: the redactors'
    warnings are logged and nothing else, so without it a user copies a command
    ...
    Enumerating tags by hand here leaves an encoded-credential-only segment
    silently rewritten and undercounts a mixed one; asking the redactor's own
    module means a newly added tag cannot escape.

    SCOPE: both body rewriters -- ``redact_credentials`` and
    ``redact_exfiltration_urls``, worded by kind because the remedies differ.
    Display-string redactions (titles, tool names, feed strings, stashed
    variants -- not text the user copies commands from) stay notice-free,
    deliberately.
    """
...
def _flush_segment(
    ...
    *,
    ...
    # Tell the user the text was altered; shared with the exception-path
    # persists via `_append_redaction_notice` so all eight
