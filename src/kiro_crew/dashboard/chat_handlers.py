...
async def api_chat_slot_reload(request: web.Request) -> web.Response:
    ...
    async with contextlib.AsyncExitStack() as _stack:
        ...
        # second. ``session_key`` cannot have gone stale from this same
        # window (it is a pure function of the slot object just reconfirmed),
        # so only the slot identity needs re-checking here.
        if state._slots.get(name) is not slot:
            return web.json_response({"error": "not found", "code": "slot_not_found"}, status=404)
