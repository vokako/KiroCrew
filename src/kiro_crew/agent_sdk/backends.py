...
#
# Named as a set rather than left as the ``is_claude_backend`` check the compaction
# branch used to spell, because that spelling handed the synchronous arm to every
# harness that is claude and withheld it from every harness that is not, without
# either being a decision anyone recorded (harness-parity H6). The membership here
# is byte-for-byte the answer that check returned, so no backend changed arms.
